    ...
    def clean(self):
        ...
        if self.user_type == "student":
            ...
            if not self.parent_email:
                raise ValidationError(
                    {"parent_email": "Parent email is required for students."}
                )
...
class FirebaseToken(models.Model):
    """Store Firebase Cloud Messaging tokens for users"""

    user = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="firebase_tokens"
    )
    token = models.TextField(unique=True)
    device_info = models.CharField(
        max_length=200, blank=True, help_text="Device/browser info"
    )
    ...
    def __str__(self):
        return f"FCM Token for {self.user.username} ({'Active' if self.is_active else 'Inactive'})"
