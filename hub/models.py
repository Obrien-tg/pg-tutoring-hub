...
class Subject(models.Model):
    ...
    def clean(self):
        ...
        if not self.color_code.startswith('#') or len(self.color_code) != 7:
            raise ValidationError({'color_code': 'Color code must be in format #RRGGBB'})
...
class Material(models.Model):
    ...
    def clean(self):
        ...
        if self.file and self.external_link:
            raise ValidationError("Provide either file upload OR external link, not both.")
...
class Assignment(models.Model):
    ...
    instructions = models.TextField(
        blank=True,
        help_text="Detailed instructions for students"
    )
